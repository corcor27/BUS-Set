...
import time
from models import GRADE_MODEL, ResNet101, SK_ResNet101, VGV
import cv2
...
train = DataGen(DATASET_PATH, "DATASET1_TRAIN", EXCEL_FILES[0], EXCEL_PATH, image_size, factor, True, True)
val = DataGen(DATASET_PATH, "DATASET1_VAL", EXCEL_FILES[1], EXCEL_PATH, image_size, factor, True, True)
test = DataGen(DATASET_PATH, "DATASET1_TEST", EXCEL_FILES[2], EXCEL_PATH, image_size, factor, False, True)
...
model = tf.keras.applications.DenseNet121(include_top=True,weights=None,input_tensor=None,input_shape=None,pooling=None,classes=2)
model.compile(optimizer=optimizer,loss="binary_crossentropy", metrics=['accuracy'])
callbacks = [tensorboard, checkpointer, ReduceLROnPlateau(monitor=('val_loss'), factor=0.7, patience=5, cooldown=0, min_lr=0)]
model.summary()
...
OUT_EXC = "RECORD/MAGVBEN_DEN.xlsx"
