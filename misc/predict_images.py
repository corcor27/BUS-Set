import numpy as np
import tensorflow as tf
from tensorflow import keras
from tensorflow.keras import Model
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Reshape, Flatten
from tensorflow.keras.layers import Input, Conv2D, Conv2DTranspose
from tensorflow.keras.layers import MaxPooling2D, Dropout, concatenate
from tensorflow.keras.layers import add, multiply, subtract
from tensorflow.keras.layers import GlobalAveragePooling2D, AveragePooling2D, UpSampling2D, Lambda
from tensorflow.keras import backend as K
from tensorflow.keras.layers import Input, Dense, Dropout, Activation, concatenate, BatchNormalization, Concatenate
from tensorflow.keras.callbacks import TensorBoard, EarlyStopping, ModelCheckpoint, ReduceLROnPlateau
import os
import sys
import random
import matplotlib.pyplot as plt
import time
from models import dice_coeff, dice_loss, dice_coef_np, voe_coef_np, selective_unet, unet, denseunet, Unetplusplus
import cv2
from data_loader import DataGen


####paths
#### 1: test set location, 2: wieghts location, 3: wights name
TRAIN_TEST_WIEGHT_SAVE = ["DATASET1_TEST","WIEGHTS/UNETPP", "UNETPP.h5", "PREDICTIONS/UNETPP/TEST"]
DATASET_PATH = "/home/a.cot12/ultrasound_deeplearning/ULTRASOUND_DATASET"
TEST_PATH = os.path.join(DATASET_PATH, TRAIN_TEST_WIEGHT_SAVE[0])
WIEGTHS_OUT_PATH = os.path.join(DATASET_PATH, TRAIN_TEST_WIEGHT_SAVE[1])
WIEGTHS_LOAD_PATH = os.path.join(WIEGTHS_OUT_PATH, TRAIN_TEST_WIEGHT_SAVE[2])
SAVE_PATH= os.path.join(DATASET_PATH, TRAIN_TEST_WIEGHT_SAVE[3])
if not os.path.exists(SAVE_PATH):
    os.mkdir(SAVE_PATH)

####parameters
image_size = 224
batch_size = 16
epochs = 100
image_channels = 1
mask_channels = 1
lr = 0.0001
SEED = 1
test = "t0"

##SEEDs

os.environ['PYTHONHASHSEED']=str(SEED)
tf.random.set_seed(SEED)
np.random.seed(SEED)
random.seed(SEED)

###paths

test_image_path = os.path.join(TEST_PATH, "images")
test_mask_path = os.path.join(TEST_PATH, "masks")
test_ids = os.listdir(test_image_path)


"""
for Datagen requires 6 inputs ids, path to images, path to masks, image size, augmentation: True/False
"""


test = DataGen(test_ids, test_image_path, test_mask_path, image_size, False)

X_test, Y_test = test.create_arg_data()
print(X_test.shape, Y_test.shape)

model = Unetplusplus()
model.summary()
METRICS = [dice_coeff, dice_loss, keras.metrics.binary_accuracy, keras.metrics.MeanIoU(num_classes=2)]
opt = keras.optimizers.Adam(lr)
model.compile(optimizer = opt, loss=[dice_loss], metrics=[METRICS])
model.load_weights(WIEGTHS_LOAD_PATH)
result = model.predict(X_test, verbose=1)
results_accuracy = (result >= 0.5).astype(np.uint8)


for ii in range(len(test_ids)):
    prediced_mask = np.squeeze(results_accuracy[ii])*255
    save_location_predict_masks = os.path.join(SAVE_PATH , test_ids[ii])
    cv2.imwrite(save_location_predict_masks, prediced_mask)
    

print("yay done")
