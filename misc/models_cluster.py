...
def GRADE_MODEL(size, dropout=0.1, filters = 16, num_grades = 3):
    ...
    return model
def ResNet101(size, num_grades = 2, filters=16):
    inp = Input(size)
...
def SK_ResNet101(size, num_grades = 2, filters=8):
    inp = Input(size)
    ...
    x = conv_block(x, 3, [filters, filters, 4*filters], strides=(1, 1))
    x = selective_identity_block(x, 3, [2*filters, 2*filters, 4*filters])
    x = selective_identity_block(x, 3, [2*filters, 2*filters, 4*filters])

    x = conv_block(x, 3, [4*filters, 4*filters, 16*filters])
    for i in range(1, 3):
        x = selective_identity_block(x, 3, [4*filters, 14*filters, 16*filters])

    x = conv_block(x, 3, [8*filters, 8*filters, 32*filters])
    for i in range(1, 20):
        x = selective_identity_block(x, 3, [8*filters, 8*filters, 32*filters])

    x = conv_block(x, 3, [16*filters, 16*filters, 64*filters])
    x = selective_identity_block(x, 3,  [16*filters, 16*filters, 64*filters])
    x = selective_identity_block(x, 3, [16*filters, 16*filters, 64*filters]) #2048
    x = AveragePooling2D((7, 7))(x)
