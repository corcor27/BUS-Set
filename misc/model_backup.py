...
def selective_unet(filters=16, drop_out=0, compression=0.5, size=(224, 224, 1),
                   half_net=False, attention_gates=True):
    inp = Input(size)
...
def Unetplusplus(size=(224, 224, 1), drop_out=0.0):
    ...
    X04 = centre_layer(X04, filters[0], drop_out)

    output = Conv2D(1, kernel_size=(1, 1), activation='softmax')(X04)
