...
import time
from models import GRADE_MODEL, ResNet101, SK_ResNet101
import cv2
...
train = DataGen(DATASET_PATH, "DATASET1_TRAIN", EXCEL_FILES[0], EXCEL_PATH, image_size, factor, False)
val = DataGen(DATASET_PATH, "DATASET1_VAL", EXCEL_FILES[1], EXCEL_PATH, image_size, factor, False)
test = DataGen(DATASET_PATH, "DATASET1_TEST", EXCEL_FILES[2], EXCEL_PATH, image_size, factor, False)
...
model = ResNet101(depth, (image_size,image_size,1))
model.compile(optimizer='adam',loss="binary_crossentropy", metrics=['accuracy'])
callbacks = [tensorboard, checkpointer, ReduceLROnPlateau(monitor=('val_loss'), factor=0.9, patience=10, cooldown=0, min_lr=0)]
model.summary()
...
OUT_EXC = "RECORD/MAGVBEN_RECORD_NET_DEPTH.xlsx"


if os.path.exists(OUT_EXC) == False:
    d = {'feature': FEATURE, 'wieghts name': WIEGHT_NAME, 'network depth': depth,'size': image_size, 'scale_factor':factor ,'batch': batch_size, 'lr': lr, 'epochs': epochs, "ACC" : score}
    d.update(TP_FP_VALS)
    data = pd.DataFrame([d])
