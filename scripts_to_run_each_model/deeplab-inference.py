...
import time
from models2 import dice_coeff, dice_loss, dice_coef_np, voe_coef_np, selective_unet, unet, denseunet, Unetplusplus,  Xception_deeplab #DeeplabV3Plus,
import cv2
import pandas as pd
from data_loader2 import DataGen
...
for fod in range(1, No_FOLDS + 1):
    ...
    if run_type == "train":
        train = DataGen(TRAIN_IDS, train_image_path, train_mask_path, image_size, augment=True, resnet_preproc=True)
        val = DataGen(VAL_IDS, train_image_path, train_mask_path, image_size, augment=False, resnet_preproc=True)
        X_train, Y_train = train.create_arg_data() ### id, aurgment true/false
        X_val, Y_val = val.create_arg_data()
        print(X_train.shape, Y_train.shape, X_val.shape, Y_val.shape)
    if run_type == "inference":
        test = DataGen(TEST_IDS, train_image_path, train_image_path, image_size, augment=False, resnet_preproc=True)
